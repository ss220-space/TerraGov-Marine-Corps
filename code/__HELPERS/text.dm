...
/proc/reject_bad_name(t_in, allow_numbers = FALSE, max_length = MAX_NAME_LEN, ascii_cyrillic_only = TRUE, with_russian = FALSE)
	if(!t_in)
	...
	for(var/i = 1, i <= t_len, i += length_char(char))
		...
		switch(char_code)
			...
			if(32)
				...
				if(with_russian)
					//А .. Я, Ё
					if((char_code >= 1040 && char_code <= 1071) || char_code == 1025)
						number_of_alphanumeric++
						last_char_group = LETTERS_DETECTED
					...
					if((char_code >= 127 && char_code <= 1024) || (char_code >= 1026 && char_code <= 1039) || char_code == 1104 || char_code >= 1106)
						if(ascii_cyrillic_only)
							return
